// Toda a lógica de main.js portada para uma função chamada via useEffect
...
const PRODUCTS = [
  {eye:{pt:'Linha Hemodinâmica',en:'Hemodynamic Line'},title:{pt:'Fio Guia PTFE',en:'PTFE Guide Wire'},img:'/assets/img/produto-fio-guia.png',desc:{pt:'Fio guia revestido em PTFE com ponta tipo J, diâmetro 0,035" e 260 cm de comprimento, indicado para facilitar a introdução e o posicionamento de cateteres em procedimentos endovasculares. O revestimento em PTFE reduz o atrito e favorece uma navegação suave.',en:'PTFE-coated guide wire with J-type tip, 0.035" diameter and 260 cm in length, designed to facilitate the introduction and positioning of catheters in endovascular procedures. The PTFE coating reduces friction for smooth navigation.'},tags:['PTFE','J-Tip','0.035" · 260 cm']},
  {eye:{pt:'Linha Hemodinâmica',en:'Hemodynamic Line'},title:{pt:'Insuflador de Balão',en:'Balloon Inflator'},img:'/assets/img/produto-insuflador.png',desc:{pt:'Insuflador de balão tipo clássico com pressão máxima de 30 ATM, utilizado para insuflar e controlar com precisão a pressão de balões em procedimentos de hemodinâmica e cardiologia intervencionista.',en:'Classic-type balloon inflator with a maximum pressure of 30 ATM, used to precisely inflate and control balloon pressure in hemodynamic and interventional cardiology procedures.'},tags:['30 ATM','Classic Type','Single Use']},
  {eye:{pt:'Linha Hemodinâmica',en:'Hemodynamic Line'},title:{pt:'Manifold Vincemed',en:'Vincemed Manifold'},img:'/assets/img/produto-manifold.png',desc:{pt:'Manifold de três vias na configuração RIGHT, estéril e de uso único, para o gerenciamento do fluxo de soluções e o controle de pressão em procedimentos hemodinâmicos. Livre de látex.',en:'Three-way manifold in RIGHT configuration, sterile and single-use, for solution flow management and pressure control in hemodynamic procedures. Latex-free.'},tags:['3-Way','RIGHT Config','Sterile']},
  {eye:{pt:'Linha Hemodinâmica',en:'Hemodynamic Line'},title:{pt:'Pulseira de Compressão Radial',en:'Radial Compression Band'},img:'/assets/img/produto-pulseira.png',desc:{pt:'Pulseira para hemostasia da artéria radial após procedimentos por acesso radial, com insuflação por seringa para compressão controlada e gradual. Estéril e de uso único.',en:'Wristband for radial artery hemostasis after radial access procedures, with syringe inflation for controlled and gradual compression. Sterile and single-use.'},tags:['Radial Access','Hemostasis','Single Use']},
  {eye:{pt:'Linha Hospitalar',en:'Hospital Line'},title:{pt:'Cateter IV Periférico',en:'Peripheral IV Catheter'},img:'/assets/img/produto-cateter.png',desc:{pt:'Cateter intravenoso periférico PRIMACAN de uso único, desenvolvido para acesso venoso periférico seguro e eficiente. Calibre 14G, com design que facilita a introdução e minimiza o trauma vascular.',en:'PRIMACAN single-use peripheral intravenous catheter, designed for safe and efficient peripheral venous access. 14G gauge with a design that facilitates insertion and minimizes vascular trauma.'},tags:['14G','PRIMACAN','IV Access']},
  {eye:{pt:'Linha Hemodinâmica',en:'Hemodynamic Line'},title:{pt:'Linha de Extensão Alta Pressão',en:'High-Pressure Extension Line'},img:'/assets/img/produto-linha-extensao.png',desc:{pt:'Linha de extensão de alta pressão com resistência de até 1200 PSI e conector valvulado, em 120 cm de comprimento. Indicada para conexão entre dispositivos em procedimentos hemodinâmicos e intervencionistas.',en:'High-pressure extension line rated up to 1,200 PSI with a valved connector, 120 cm in length. Designed for device connection in hemodynamic and interventional procedures.'},tags:['1200 PSI','Valved Connector','120 cm']},
  {eye:{pt:'Linha Hemodinâmica',en:'Hemodynamic Line'},title:{pt:'Torneira de 3 Vias',en:'3-Way Stopcock'},img:'/assets/img/produto-torneira.png',desc:{pt:'Torneira de três vias para controle e direcionamento do fluxo de soluções em procedimentos de cateterismo e hemodinâmica. Livre de látex, estéril e de uso único.',en:'Three-way stopcock for flow control and direction in catheterization and hemodynamic procedures. Latex-free, sterile, and single-use.'},tags:['3-Way','Latex-Free','Single Use']},
  {eye:{pt:'Linha Hemodinâmica',en:'Hemodynamic Line'},title:{pt:'Válvula Hemostática',en:'Hemostatic Valve'},img:'/assets/img/produto-valvula.png',desc:{pt:'Válvula hemostática com conectores Y e rosca para vedação hermética durante procedimentos endovasculares. Permite a passagem de guias e cateteres com mínimo sangramento.',en:'Hemostatic valve with Y and threaded connectors for hermetic sealing during endovascular procedures. Allows guide wires and catheters to pass through with minimal blood loss.'},tags:['Y Connector','Threaded','Single Use']},
]
...
export function initHome() {
  ...
  PRODUCTS.forEach((p, i) => {
    ...
    c.innerHTML = `<img src="${p.img}" alt="${p.title.pt}"><div class="parrow"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M13 6l6 6-6 6"/></svg></div>`
    ...
    d.addEventListener('click', () => { idx = i; render(); startAuto() })
    dotsEl.appendChild(d)
  })

  const cards = [...stage.children], dotEls = [...dotsEl.children]
  ...
  function navState() {
    const inHero = heroEl ? window.scrollY < heroEl.offsetHeight - 90 : false
    const atTop = window.scrollY < 10
    nav?.classList.toggle('over', inHero)
    nav?.classList.toggle('at-top', atTop)
    nav?.classList.toggle('scrolled', !inHero)
  }
  ...
  function setLang(lang) {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => { const v = T[lang][el.dataset.i18n]; if (v !== undefined) el.textContent = v })
    document.querySelectorAll('[data-i18n-ph]').forEach(el => { const v = T[lang][el.dataset.i18nPh]; if (v !== undefined) el.placeholder = v })
    document.querySelectorAll('.lang button').forEach(b => b.classList.toggle('on', b.textContent.toLowerCase() === lang))
    render()
    if (typeof rebuildManifesto === 'function') rebuildManifesto(lang)
    localStorage.setItem('vincemed-lang', lang)
  }
  document.querySelectorAll('.lang button').forEach(b => b.addEventListener('click', function () { setLang(this.textContent.toLowerCase()) }))
  ...
  ;(function () {
    ...
    function build(lang) {
      ...
      if (_noMotion) spans.forEach(s => s.style.opacity = 1)
    }
    function reveal() {
      if (_noMotion) return
  ...
  ;(function () {
    ...
    function setLoading(on) { btn.disabled = on; btn.classList.toggle('loading', on); btn.querySelector('.btn-txt').textContent = on ? T[currentLang]['form.sending'] : T[currentLang]['form.submit'] }
    function showFeedback(type, msg) { feedback.textContent = msg; feedback.className = 'cf-feedback cf-' + type; feedback.hidden = false }
    form.addEventListener('submit', async e => {
      ...
      try {
        const res = await fetch('/api/contact', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nome, empresa, email, telefone, assunto, mensagem }) })
      ...
      } catch { showFeedback('error', T[currentLang]['form.err.send']) }
